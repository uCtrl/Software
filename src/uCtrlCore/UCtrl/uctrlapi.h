...
public:
    explicit UCtrlAPI(QNetworkAccessManager* nam, QObject *parent = 0);
...
private:
    ...
    bool checkModel(UPlatform* platform);
    bool checkModel(UDevice* device);
    bool checkModel(UScenario* scenario);
    bool checkModel(UTask* task);
    bool checkModel(UCondition* condition);
